	...
	for groupOne := 0; groupOne < 10; groupOne++ {
		...
		for groupTwo := 0; groupTwo < 10; groupTwo++ {
			...
			for groupThree := 0; groupThree < 10; groupThree++ {
				...
				if groupOne != groupTwo && groupOne != groupThree && groupTwo != groupThree {
...
func checkFourOnLeft(wg *sync.WaitGroup, data []movieTitleActorNaive) {
	...
	for groupOne := 0; groupOne < 10; groupOne++ {
		...
		numMap[groupOne] = true

		for groupTwo := 0; groupTwo < 10; groupTwo++ {

			_, ok := numMap[groupTwo]

			if !ok {
				continue
			}

			numMap[groupTwo] = true

			for groupThree := 0; groupThree < 10; groupThree++ {

				_, ok := numMap[groupThree]

				if !ok {
					continue
				}

				numMap[groupThree] = true

				for groupFour := 0; groupFour < 10; groupFour++ {

					_, ok := numMap[groupFour]

					if !ok {
						continue
					}

					numMap[groupFour] = true
